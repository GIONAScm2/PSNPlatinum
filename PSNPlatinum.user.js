...
(async () => {
    ...
    ******************************************************************************************************************************/
    ...
    /** PROFILE[?search=] */
    async function enhanceProfilePages() {
        class PSNProfile {
            ...
            static async envy() {
                ...
                for (let i = 1; i <= profile.numPages; i++) {
                    ...
                    for (let j = 0; j < gameNodes.length; j++) {
                        const game = new GameWithProgress(gameNodes[j]);
                        if (!Settings.games.has(game.id)) table.appendChild(game.el);
                    }
                    monitorGames();
                }
            }
    ...
    async function enhanceTrophyList() {
    ...
    async function enhanceTrophyPage() {
    ...
    async function enhanceForumThread() {
